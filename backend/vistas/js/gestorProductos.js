...
$.ajax({
	...
	url:"ajax/tablaProductos2.ajax.php",
...
$('.tablaProductosModal tbody').on("click", ".btnGrupoAsignar", function(event){
	...
	if($("#codigoCombo").val()==""){
		...
		$("#codigoCombo").val(codigo);

	}else{

		$("#codigoCombo").val($("#codigoCombo").val()+"-"+codigo);

	}

	if($("#detalleCombo").val()==""){

		$("#detalleCombo").val(detalle);

	}else{

		$("#detalleCombo").val($("#detalleCombo").val()+" "+detalle);

	}

	if($("#importeCombo").val()==""){

		$("#importeCombo").val(precio);

	}else{

		total = Number($("#importeCombo").val());
		precio = Number(precio);
		importe_total = total +  precio;
		$("#importeCombo").val(importe_total);
...
ACTIVAR PRODUCTO
=============================================*/
$('.tablaProductos tbody').on("click", ".btnAgregarFotoProducto", function(){
	...
	$.ajax({
		...
		success: function(respuesta){
			...
			$("#rutaProducto").val(respuesta[0]["ruta"]);
...
$('.tablaProductos').DataTable({
	...
	// "ajax":"ajax/tablaProductos2.ajax.php",
	"deferRender": true,
	"retrieve": true,
	"processing": true,
    "language": {
...
/*=============================================
HACER FOCO EN NOMBRE DE LA VENTANA MODAL DE NUEVO
=============================================*/
$('#modalEditarPrecioProducto').on('shown.bs.modal', function () {
    
    $('#importeProducto').select();
...
ACTIVAR PRODUCTO
=============================================*/
$('.tablaProductos tbody').on("click", ".btnVista", function(){

	var idProducto = $(this).attr("idProducto");
	console.log("idProducto", idProducto);
	var vistaProducto = $(this).attr("vistaProducto");
	console.log("vistaProducto", vistaProducto);

	var datos = new FormData();
 	datos.append("idProductoVista", idProducto);
  	datos.append("vistaProducto", vistaProducto);
	...
	if(vistaProducto != "productos"){

  		$(this).removeClass('btn-success');
  		$(this).addClass('btn-danger');
  		$(this).html('Productos');
  		$(this).attr('vistaproducto',"productos");
  		...
  		$(this).removeClass('btn-danger');
  		$(this).html('Catalogo');
  		$(this).attr('vistaproducto',"catalogo");
...
$("#btnComboNuevo").click(function(){

	console.log("id", $("#idcombo").val());

})

$("#cantidad").change(function(event) {
	
	precio1 = $("#precio1_oculto").val();
	precio2 = $("#precio2_oculto").val();

	precio1 = precio1 * $(this).val();
	precio2 = precio2 *$("#cantidad2").val();

	precioTotal = parseInt(precio1) + parseInt(precio2);

	$("#precioautomatico").val(precioTotal);

});

$("#cantidad2").change(function(event) {
	
	precio1 = $("#precio1_oculto").val();
	console.log("precio1", precio1);
	precio2 = $("#precio2_oculto").val();

	precio1 = precio1 *$("#cantidad").val();
	console.log("precio1", precio1);
	precio2 = precio2 * $(this).val();
...
CAMBIAR 
=============================================*/
$("#tablaCombos").on("click", ".btnSeleccionarCombo", function(){
	//ESCONDO LA PRIMERA TABLA
	$("#tablita").hide('slow');
	//MUESTRO LOS DATOS PARA AGREGAR
	$("#datoscombo").show('slow');
	//MUESTRO LA SEGUNDA TABLA
	$("#tablita2").show('slow');
	// TOMO LOS VALORES DEL BOTON
	var idProductoCombo = $(this).attr("id");
	var producto1 = $(this).attr("producto1");
	var precio = $(this).attr("precio");
	precioTotal = precio;
	//LOS CARGO EN LOS INPUTS
	$("#idcombo").val(idProductoCombo);
	$('#producto1').val(producto1);
	$('#precio1_oculto').val(precio);
	$('#precio1').val(precio);
	$('#precioautomatico').val(precioTotal);
	//PONGO EN SOLO LECTURA
	$('#idcombo').prop('readonly', true);
	$('#producto1').prop('readonly', true);
...
CAMBIAR 
=============================================*/
$("#tablaCombos2").on("click", ".btnSeleccionarCombo2", function(){
	//ESCONDO LA SEGUNDA TABLA
	$("#tablita2").hide('slow');
	// TOMO LOS VALORES DEL BOTON
	var idProductoCombo2 = $(this).attr("id");
	var producto2 = $(this).attr("producto2");
	var precio = $(this).attr("precio");
	//LOS CARGO EN LOS INPUTS
	$("#idcombo2").val(idProductoCombo2);
	$('#producto3').val(producto2);
	$('#precio2_oculto').val(precio);
	$('#precio2').val(precio);
	precioTotal = parseInt(precioTotal)+parseInt(precio);
	$('#precioautomatico').val(precioTotal);
	//PONGO EN SOLO LECTURA
	$('#idcombo2').prop('readonly', true);
	$('#producto3').prop('readonly', true);
...
$("#listadoProductos").change(function(){

	var tipoVista = $(this).val();
	
	if (tipoVista==0){window.location = "productos2"};
	if (tipoVista=="productos") {window.location = "index.php?ruta=productos2&tipo="+tipoVista};
	if (tipoVista=="catalogo"){window.location = "index.php?ruta=productos2&tipo="+tipoVista};
...
/*=============================================
SUBIENDO LA FOTO DE TAPA
=============================================*/

var imagenTapa = null;

$(".fotoTapa").change(function(){

	imagenTapa = this.files[0];
	
	/*=============================================
  	VALIDAMOS EL FORMATO DE LA IMAGEN SEA JPG O PNG
  	=============================================*/

  	if(imagenTapa["type"] != "image/jpeg" && imagenTapa["type"] != "image/png"){
  	...
  	}else if(imagenTapa["size"] > 4000000){
  		...
  		datosImagen.readAsDataURL(imagenTapa);

  		$(datosImagen).on("load", function(event){

  			var rutaImagen = event.target.result;

  			$(".previsualizarTapa").attr("src", rutaImagen);
...
$(".fotoOferta").change(function(){
	...
	/*=============================================
  	...
  	}else if(imagenOferta["size"] > 4000000){
...
$('.tablaProductos tbody').on("click", ".btnEditarPrecioProducto", function(){

	var idPrecioProducto = $(this).attr("idPrecioProducto");
	var codigo = $(this).attr("codigo");
	var precio = $(this).attr("precio");

	$("#modalEditarPrecioProducto #idPrecioProducto").val(idPrecioProducto);
	$("#modalEditarPrecioProducto #codigoProducto").val(codigo);
	$("#modalEditarPrecioProducto #importeProducto").val(precio);
		...
		success: function(respuesta){
			...
			$.ajax({
					...
					success: function(respuesta){
						...
						$("#modalEditarProducto .antiguaFotoTapa").val(respuesta["image_tabla"]);
					   ...
					   $("#modalEditarProducto .pClavesProducto").val() != ""){
						...
						/*=============================================
					   	...
					   	if($("#modalEditarProducto .seleccionarTipo").val() != "virtual"){	
					   		console.log("PREGUNTAMOS SI VIENEN IMÁGENES PARA MULTIMEDIA O LINK DE YOUTUBE", "VIENEN IMAGENES");
console.log("arrayFiles", arrayFiles);
console.log("$(\"#modalEditarProducto .rutaProducto\").val()", $("#modalEditarProducto .rutaProducto").val());
...
function editarMiProducto(imagen){
	...
	var antiguaFotoTapa = $("#modalEditarProducto .antiguaFotoTapa").val();
	...
	datosProducto.append("antiguaFotoTapa", antiguaFotoTapa);
...
$('.tablaProductos tbody').on("click", ".btnEliminarProducto", function(){
  ...
  swal({
    ...
    if(result.value){
      ...
      window.location = "index.php?ruta=productos2&idProducto="+idProducto+"&imgOferta="+imgOferta+"&rutaCabecera="+rutaCabecera+"&imgPortada="+imgPortada+"&imgPrincipal="+imgPrincipal;
...
$('.tablaProductos tbody').on('click','button.btnImagenPortada',function(){
	
	var imgPortada = $(this).attr("imgportada");

	$("#imagenVerPortada").attr("src",imgPortada);
