...
<div class="container-fluid infoproducto">
	...
	// echo '<center><pre>'; print_r($infoproducto); echo '</pre></center>';
				if(empty($infoproducto)){

					$infoproducto = ControladorProductos::ctrMostrarInfoListado($item, $valor);
				...
				// if($infoproducto["tipo"] == "fisico"){
					...
					echo '<div class="col-md-5 col-sm-6 col-xs-12 visorImg">
							...
							<figure class="visor">';
								...
								if($infoproducto["vista"] == "catalogo"){

									$imagenCatalogo=$infoproducto["imagen_tabla"];
// echo $imagenCatalogo;
										if(file_exists($imagenCatalogo)){

											echo '<img id="lupa1" class="img-thumbnail" src="'.$infoproducto["imagen_tabla"].'">';
										
										}else{
											
											echo '<img id="lupa1" class="img-thumbnail" src="'.$servidor.$multimedia[0]["foto"].'">';
				...
				<?php
					...
					echo '<div class="comprarAhora" style="display:none">
						...
						if($infoproducto["oferta"] == 0){
							...
							echo'<input class="cantidadItem" value="1" tipo="'.$infoproducto["tipo"].'" precio="'.ceil($infoproducto["precioOferta"]).'" idProducto="'.$infoproducto["id"].'">

							<p class="subTotal'.$infoproducto["id"].' subtotales">
						
								<strong> $<span>'.ceil($infoproducto["precioOferta"]).'</span></strong>

							</p>

							<div class="sumaSubTotal"><span>'.ceil($infoproducto["precioOferta"]).'</span></div>';
					...
					if($infoproducto["oferta"] == 0){
						...
						if($fechaNueva > $infoproducto["fecha"]){
							...
							<small>
								...
								<span class="label label-warning productoNuevo">Nuevo</span>
						...
						if($fechaNueva > $infoproducto["fecha"]){
							...
							if($infoproducto["precio"] != 0){
								...
								echo '<small>
									<span class="label label-warning productoNuevo">Nuevo</span> 
									<span class="label label-warning productosOff">'.$infoproducto["descuentoOferta"].'% off</span> 
						...
						if($infoproducto["oferta"] == 0){
							...
							echo '<h2 class="text-muted">
								...
								<span>
									...
									$'.ceil($infoproducto["precioOferta"]).'
...
<?php
						...
						if($infoproducto["tipo"]=="virtual"){
							...
							echo '</div>
								...
								if($infoproducto["oferta"] != 0){
									...
									echo '<button class="btn  btn-block btn-lg productosOff agregarCarrito"  idProducto="'.$infoproducto["id"].'" imagen="'.$servidor.$infoproducto["portada"].'" titulo="'.$infoproducto["titulo"].'" precio="'.ceil($infoproducto["precioOferta"]).'" tipo="'.$infoproducto["tipo"].'" peso="'.$infoproducto["peso"].'">';

								}else{

									echo '<button class="btn  btn-block btn-lg productosOff agregarCarrito"  idProducto="'.$infoproducto["id"].'" imagen="'.$servidor.$infoproducto["portada"].'" titulo="'.$infoproducto["titulo"].'" precio="'.$infoproducto["precio"].'" tipo="'.$infoproducto["tipo"].'" peso="'.$infoproducto["peso"].'">';
							...
							if($infoproducto["oferta"] != 0){
									...
									echo '<button class="btn  btn-block btn-lg btnCarritoCss agregarCarrito"  idProducto="'.$infoproducto["id"].'" imagen="'.$servidor.$infoproducto["portada"].'" titulo="'.$infoproducto["titulo"].'" precio="'.ceil($infoproducto["precioOferta"]).'" tipo="'.$infoproducto["tipo"].'" peso="'.$infoproducto["peso"].'">';

								}else{

									echo '<button class="btn  btn-block btn-lg btnCarritoCss agregarCarrito"  idProducto="'.$infoproducto["id"].'" imagen="'.$servidor.$infoproducto["portada"].'" titulo="'.$infoproducto["titulo"].'" precio="'.$infoproducto["precio"].'" tipo="'.$infoproducto["tipo"].'" peso="'.$infoproducto["peso"].'">';
			...
			<?php
				...
				if($cantidad == 0){
					...
					echo '<li class="active"><a>COMENTARIOS '.$cantidad.'</a></li>
						  <li><a id="verMas" href="" class="btnCarritoCss">Ver más</a></li>';
		...
		<div class="row" style="background: white;margin-bottom: 0px;">
...
======================================-->
<?php if ($infoproducto["id_subcategoria"]!=0): ?>
	
<div class="container-fluid productos" style="background: white;">
	...
	<div class="container">
				...
				<div class="col-sm-6 col-xs-12">
					...
					<?php
						...
						echo '<a href="'.$url.$rutaArticulosDestacados[0]["ruta"].'">
							...
							<button class="btn  btnCarritoCss pull-right">
		...
		<?php
			...
			$relacionados = ControladorProductos::ctrMostrarProductos($ordenar, $item, $valor, $base, $tope, $modo,$tabla);
			...
			if(!$relacionados){
				...
				if($value["estado"] != 0 && $value["precio"] > 0 && $value["stock"] > 2){
					...
					echo '<li class="col-md-3 col-sm-6 col-xs-12">
						...
						$imagenCatalogo=$value["imagen_tabla"];
						
					
						if(file_exists($imagenCatalogo)){//trato de poner la imagen global
					
							echo '<img src="'.$imagenCatalogo.'" class="img-responsive">';
						...
						<div class="col-xs-6 enlaces">
							...
							<div class="btn-group pull-right">
								...
								<button type="button" class="btn  btn-xs deseos" idProducto="'.$value["id"].'" data-toggle="tooltip" title="Agregar a mi lista de deseos">
								...
								if($value["tipo"] == "virtual" && $value["precio"] != 0 && $value["stock"] > 2){
									...
									if($value["oferta"] != 0){
										...
										echo '<button type="button" class="btn  btn-xs agregarCarrito"  idProducto="'.$value["id"].'" imagen="'.$servidor.$value["portada"].'" titulo="'.$value["titulo"].'" precio="'.$value["precioOferta"].'" tipo="'.$value["tipo"].'" peso="'.$value["peso"].'" data-toggle="tooltip" title="Agregar al carrito de compras">
										...
										echo '<button type="button" class="btn  btn-xs agregarCarrito"  idProducto="'.$value["id"].'" imagen="'.$servidor.$value["portada"].'" titulo="'.$value["titulo"].'" precio="'.$value["precio"].'" tipo="'.$value["tipo"].'" peso="'.$value["peso"].'" data-toggle="tooltip" title="Agregar al carrito de compras">
								...
								echo '<a href="'.$url.$value["ruta"].'" class="pixelProducto">
									...
									<button type="button" class="btn  btn-xs" data-toggle="tooltip" title="Ver producto">
