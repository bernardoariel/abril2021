...
$titulosModulos = array("CATALOGO DEL MES", "LO MÁS VISTO","MÁS PROMOS");//"LO MÁS VENDIDO",

$rutaModulos = array("catalogo","lo-mas-visto","promos");//"lo-mas-vendido",
...
if($titulosModulos[2] == "MÁS PROMOS"){

	$base = 0;
	$tope = 4;

	$ordenar = "tipo";
	$item = "grupo"; //ESTADO=0 NO ESTA ACTIVO ESTADO=1 ESTA ACTIVO CON FOTO ESTADO 2 ESTA ACTIVO SIN FOTO NO SALE EN LA PORTADA
	$valor = 1;
	$modo = "DESC";
	$tabla="productos";

	$promos = ControladorProductos::ctrMostrarProductosCatalogoGrupos($base, $tope);

}


$modulos = array($ventas,$vistas,$promos);
							...
							<figure style="border-width: 1px;border-style: solid;border-color: #D8D8D8;">
								...
								<a href="'.$value["ruta"].'" class="pixelProducto">
									...
									if($value["multimedia"]!=""){
										...
										// $imagenCatalogo=$servidor.$value["imagen_tabla"];
										$imagenCatalogo="../backend/".$value["imagen_tabla"];
										// echo $imagenCatalogo;//$value["imagen_tabla"];
										// $pe= file_exists($imagenCatalogo);
										// echo '<center><pre>'; print_r($pe); echo '</pre></center>';
									
										if(file_exists($imagenCatalogo)){//trato de poner la imagen global
									
											echo '<img src="'.$imagenCatalogo.'" class="img-responsive" width="100%">';

										}else{

											$imagenListado=$servidor."vistas/img/listado/".$value["ruta"]."/".$value["ruta"].".jpg";

											echo '<img src="'.$imagenListado.'" class="img-responsive" width="100%">';
